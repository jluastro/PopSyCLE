...
def perform_pop_syn(ebf_file, output_root, iso_dir,
    ...
    for pid in range(10):
        ...
        for aa in range(len(logt_bins) - 1):
            ...
            if len(age_idx) <= 100: #FIXME
                continue

            #break each age bin into 5 metallicity bins based on a cdf generated by the metallicity_age_bin array
            feh_counts, feh_bin_edges = np.histogram(metallicity_age_bin, 10000, density=True)
            feh_cum_sum = np.cumsum(feh_counts)
            feh_cdf = feh_cum_sum/feh_cum_sum[-1]
            print(feh_cdf)
            feh_bins = np.zeros(6)
            feh_bins[0] = min(metallicity_age_bin)
            for i in range(1, 5):
                cumulation = i*0.2
                idx = np.where((feh_cdf <= (cumulation + 0.01)) & (feh_cdf > (cumulation - 0.01)))
                feh_bins[i]= feh_bin_edges[idx[0][0]]
            feh_bins[5] = max(metallicity_age_bin)
            print(feh_bins) #FIXME

            for bb in range(len(feh_bins) - 1):
                print('Starting metallicity bin ', feh_bins[bb])
                # Midpoint of metallicity bin
                metallicity_of_bin = (feh_bins[bb] + feh_bins[bb + 1]) / 2.0
